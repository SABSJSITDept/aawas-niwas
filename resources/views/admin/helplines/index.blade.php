...
<div class="p-6 font-['Inter']">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8 bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-2xl font-bold text-slate-800">हेल्पलाइन प्रबंधन</h1>
                <p class="text-sm text-slate-500 font-medium">Manage all contact numbers and helplines</p>
            </div>
        </div>
        <div>
            <a href="{{ route('admin.helplines.create') }}" class="inline-flex items-center gap-2 px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-semibold rounded-xl shadow-sm hover:shadow transition-all">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
        ...
        <div class="p-12 text-center flex flex-col items-center">
            ...
            <p class="text-slate-500 mt-1">अभी तक कोई हेल्पलाइन नंबर नहीं जोड़ा गया है।</p>
        </div>
        @else
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-slate-50/80 border-b border-slate-100">
                        <th class="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider">श्रेणी (Category)</th>
                        <th class="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider">प्रतिनिधि</th>
                        <th class="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider">नंबर</th>
                        <th class="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider text-center">होम मेडिकल?</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr class="hover:bg-slate-50/50 transition-colors">
                        ...
                        <td class="px-6 py-4 text-slate-600 font-medium">
                            <i class="bi bi-telephone text-slate-400 mr-2"></i>{{ $item->number }}
                        </td>
                        <td class="px-6 py-4 text-center">
                            @if($item->is_home_medical)
                                <span class="inline-flex items-center justify-center w-8 h-8 rounded-lg bg-rose-50 text-rose-600" title="Home Medical Number">
                        ...
                        <td class="px-6 py-4 text-right">
                            <div class="flex items-center justify-end gap-2">
                                <form action="{{ route('admin.helplines.toggle', $item->id) }}" method="POST" class="inline">
                                    @csrf
                                    @method('PATCH')
                                ...
                                <a href="{{ route('admin.helplines.edit', $item->id) }}" class="w-8 h-8 rounded-lg bg-blue-50 text-blue-600 hover:bg-blue-100 flex items-center justify-center transition-colors" title="Edit">
                                    <i class="bi bi-pencil-square"></i>
                                </a>
                                <form action="{{ route('admin.helplines.destroy', $item->id) }}" method="POST" class="inline" onsubmit="return confirm('क्या आप वाकई इसे हटाना चाहते हैं?');">
                                    @csrf
                                    @method('DELETE')
