...
@section('title', 'All Registrations (API)')
...
<div class="container-fluid py-4">
    <div class="page-header">
        <div class="d-flex justify-content-between align-items-center">
            <div>
                <h2><i class="fas fa-list me-2"></i>All Registrations</h2>
                <p class="subtitle mb-0">View all bookings (pending, allotted, checkout, rejected)</p>
            </div>
...
<script>
const API_URL = '/api/registration/all-list';
let currentPage = 1;
...
const DB_COLUMNS = [
    ...
    { key: 'status', exportKey: 'status', label: 'Status', defaultVisible: true, filterable: true, type: 'select', options: ['pending', 'completed', 'check-out', 'rejected'] },
...
function initDynamicHeaders() {
    ...
    // We will build the table in renderTable, so we can't inject headers yet if table doesn't exist
    // Actually renderTable rewrites the whole table. So we will just build the thead inside renderTable using DB_COLUMNS
}
    ...
    data.forEach((row, i) => {
        ...
        DB_COLUMNS.forEach((col, idx) => {
            ...
            if (col.key === 'index') val = ((currentPage - 1) * 25) + i + 1;
                ...
                else if (row.status === 'check-out') badgeClass = 'bg-info text-dark';
                else if (row.status === 'rejected') badgeClass = 'bg-danger text-white';
                val = `<span class="status-badge ${badgeClass}">${(row.status || 'pending').toUpperCase()}</span>`;
            }
            else if (col.key === 'actions') {
                if (row.status === 'completed') {
                    val = `<div class="d-flex gap-2"><button class="btn btn-primary btn-sm" onclick="changeRoom('${row.type}', ${row.id})">Change Room</button><button class="btn btn-danger btn-sm" onclick="checkoutBooking('${row.type}', ${row.id})">Check Out</button></div>`;
                } else if (row.status === 'pending') {
                    val = `<a href="/alot-room?booking_id=${row.id}&booking_type=${row.type}" class="btn btn-success btn-sm">Allot Room</a>`;
                } else {
                    val = '-';
                }
            }
...
async function exportCompletedData(format) {
    try {
        ...
        const exportUrl = `/api/registration/all-export?${params.toString()}`;
        ...
        link.href = url;
        link.download = `all-registrations-${new Date().toISOString().slice(0,10)}.${format === 'excel' ? 'xlsx' : 'pdf'}`;
